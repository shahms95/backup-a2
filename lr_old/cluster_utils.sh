...
function start_cluster() {
    if [ -z $2 ]; then
        ...
        if [ "$2" = "single" ]; then
            ...
            nohup ssh node0 "cd ~/tf ; python $1 --deploy_mode=cluster  --job_name=ps --sync=$3" > serverlog-ps-0.out 2>&1&
            nohup ssh node0 "cd ~/tf ; python $1 --deploy_mode=cluster  --task_index=0 --sync=$3" > serverlog-0.out 2>&1&
            nohup ssh node1 "cd ~/tf ; python $1 --deploy_mode=cluster  --task_index=1 --sync=$3" > serverlog-1.out 2>&1&
        else
            nohup ssh node0 "cd ~/tf ; python $1 --deploy_mode=cluster2  --job_name=ps --sync=$3" > serverlog-ps-0.out 2>&1&
            nohup ssh node0 "cd ~/tf ; python $1 --deploy_mode=cluster2  --task_index=0 --sync=$3" > serverlog-0.out 2>&1&
            nohup ssh node1 "cd ~/tf ; python $1 --deploy_mode=cluster2  --task_index=1 --sync=$3" > serverlog-1.out 2>&1&
            nohup ssh node2 "cd ~/tf ; python $1 --deploy_mode=cluster2  --task_index=2 --sync=$3" > serverlog-2.out 2>&1&
        fi
    fi
}
